...
function annotatePrimitiveSlide(slide, si) {
  const elements = (slide.elements || []).map((el, ei) => ({
    ...
    ...(el.elType === 'text' ? { sourcePath: el.sourcePath || `/slides/${si}/elements/${ei}/text` } : {}),
  }));
...
export function compileDeck(deck, opts = {}) {
  ...
  const slides = source.slides.map((slide, si) => isSemanticSlide(slide)
    ? expandLayoutSlide(slide, { slideIndex: si, pageCount, theme })
    : annotatePrimitiveSlide(slide, si));
  let compiled = {
