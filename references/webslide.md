...
- 非方形节点的像素半高圆角保留为胶囊矩形，不会误转为椭圆。
- 普通二维旋转使用元素框中心及实际边长；祖先二维变换纳入几何计算，透明度相乘。
- `display:none` / `visibility:hidden` 元素不输出，原因记入 `slide.webOmitted`，不伪装成零尺寸元素。
...
- 祖先 overflow 裁切越界内容、嵌套堆叠上下文、多个子元素的组透明度合成。
- 倾斜、镜像、3D 变换、CSS 独立 rotate/scale/translate 属性和非均匀缩放页面。
- 未标记容器上的可见背景、非统一圆角及非居中图片 object-position。

这些项目会告警而不是自动栅格化；构建者需明确接受近似或修改输入。常规旋转请使用 `transform:rotate(...)`，复杂裁切可将局部装饰预合成为图片，文字保留为单独可编辑对象。
