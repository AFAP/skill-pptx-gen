...
以下是可选构图起点，不是设计规则或转换契约。所有坐标基于 1280×720；使用后仍需校验和渲染，按实际内容调整。

按需搜索：`字号层级`、`配色规则`、`脑图`、`弧形轨道`、`四象限`、`金字塔`、`SWOT`、`三环循环`、`数据仪表盘`、`禁用清单`。普通语义版式任务无需整份读取。
...
| H1 | 28-42 | 页面标题（常规页 28-32，强调页 36-42） |
| H2 | 24-28 | 副标题、区块标题 |
| H3 | 20-26 | 卡片标题 |
| Body | 14-18 | 正文（12px 仅限极短辅助文案，不建议作为正文字号） |
| Caption | 10-13 | 注释、来源、页码（不低于 10） |

英文小标签（如 "CONTENTS"）：12px + `letterSpacing: 3`；浅底使用 `$accentText`，深底只有对比度合格时才使用 `$accent`。

## 节奏与密度

- 要点、字数和标题长度由信息关系、实际排版和用户要求决定，不设统一硬上限。
- 内容关系变化时可以改变布局；不按固定页数强制切换风格。
- 内容多宁分页不堆砌。
...
- 浅色背景上的强调文字使用 `$accentText`；`$accent` 用于装饰、描边、填充和图表系列；accent 色块上的文字使用 `$onAccent`。
- palette 色直接作为文字前必须通过对比度检查；不通过时改用 `$primary`、`$text` 或 `$accentText`。
- 中文证券/投资语境默认红色表示上涨、绿色表示下跌；非金融场景不要套用这一约定。
...
```
标题组同上
大数字卡 (N 个): 卡宽 = (1280 - 120 - 24×(N-1))/N，x = 60 + i×(卡宽+24)，高 180；例如 4 卡时卡宽 272、x=60/356/652/948
...
以下配方来自原项目。常见连接线可以用 `connector-s` / `connector-elbow` / `arc-segment` 宏降低 token；特殊曲线可以使用明确的 pointArr，不限制 AI 构图。
...
注意：原版环上的弧形文字标签（收益平衡…）若用 text-path 只能预览，严格导出会报错；
  需要出片时改为各象限内的直排标签。
```
...
每卡: shape-rect 高200 fill=palette[i]+"15" stroke=palette[i] 2px cornerRadius 16
  大数字 48-72px bold；palette[i] 对比度不足时改用 $primary/$accentText + 标签 16px + 趋势 14px（按语境使用涨跌色）
```
...
检查文字是否清晰、内容是否完整、装饰是否遮挡信息。色彩数量、字数、留白和阴影由实际任务决定。渐变、字体和裁切的转换边界以 parity-contract.md 为准，不能用设计偏好替代能力检查。
